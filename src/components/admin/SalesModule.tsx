        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            ...
            <div className="flex gap-3">
                <button onClick={() => setView('pos')}
                    className={`px-6 py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${view === 'pos' ? 'bg-admin-btn text-white shadow-lg shadow-black/10' : 'bg-admin-card border border-admin-border text-admin-primary hover:bg-admin-bg'}`}>
                    <Calculator className="w-4 h-4 inline mr-2" /> Point de Vente
                </button>
                <button onClick={() => setView('orders')}
                    className={`px-6 py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${view === 'orders' ? 'bg-admin-btn text-white shadow-lg shadow-black/10' : 'bg-admin-card border border-admin-border text-admin-primary hover:bg-admin-bg'}`}>
                    ...
                    <div className="overflow-x-auto custom-scrollbar">
                        <table className="w-full">
                            ...
                            <tbody className="divide-y divide-admin-border">
                                {orders.map(order => (
                                    <tr key={order.id} className="hover:bg-admin-bg/30 transition-colors">
                                        <td className="px-6 py-5">
                                            <span className="text-xs font-mono-tech text-admin-btn font-bold">{order.order_number}</span>
                                        </td>
                                        <td className="px-6 py-5">
                                            <div className="text-sm font-bold text-admin-title">{order.customer_name}</div>
                                            <div className="text-[10px] text-admin-secondary font-medium uppercase tracking-tighter">{order.customer_phone || order.customer_email || ''}</div>
                                        </td>
                                        <td className="px-6 py-5">
                                            <span className="text-sm font-mono-tech font-bold text-admin-primary">{Number(order.total_amount).toLocaleString()} DA</span>
                                        </td>
                                        <td className="px-6 py-5"><StatusBadge status={order.status} /></td>
                                        <td className="px-6 py-5">
                                            <span className={`text-[9px] font-bold uppercase tracking-widest ${order.is_pos_sale ? 'text-admin-btn' : 'text-blue-600'}`}>
                                                {order.is_pos_sale ? 'POS' : 'Online'}
                                            </span>
                                        </td>
                                        <td className="px-6 py-5">
                                            <span className="text-[10px] text-admin-secondary font-mono-tech font-medium">
                                        ...
                                        <td className="px-6 py-5">
                                            {order.status !== 'delivered' && order.status !== 'cancelled' && (
                                                <select
                                                    ...
                                                    className="bg-admin-bg border border-admin-border text-[10px] font-bold text-admin-primary px-3 py-1.5 rounded-lg outline-none focus:border-admin-secondary/40 transition-all uppercase tracking-widest">
